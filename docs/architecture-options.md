...
| **2. Overlay + автономный сервис (`plain` + `:api-app`)** | Модульность; независимое обновление UI-оверлея и бэкенда; доступность бэкенда другим приложениям | Требует установки двух отдельных APK и настройки двух наборов разрешений | **Поддерживаемый модульный вариант / Supported Modular** |

...
Все четыре клиентских пункта реализованы в версии `1.0.2 (3)`. Проверки firmware-specific частей
...
- waveform-заглушка в свободной верхней зоне, когда обложка отсутствует;
...
`1.0.2[3]AtlasMediaWidget`. Без локального `secure.signing.gradle` release APK остаётся unsigned.

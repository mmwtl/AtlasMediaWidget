...
`protocol v1` автономного сервиса Atlas Media API (`com.mmwtl.atlasmediaapi`).
...
- поддержка обложек и названий станций радио из каталога Atlas Media API;
...
- foreground service, автозапуск после загрузки ГУ и восстановление соединения с Atlas Media API;
...
- актуальная Atlas Media API (`com.mmwtl.atlasmediaapi`) с поддержкой Media Bridge `protocol v1`,
  `GET_RADIO_STATIONS` и `TUNE_RADIO`;
...
1. Установите и настройте Atlas Media API.
...
4. Проверьте статус Atlas Media API на экране настроек.
...
централизованно в сервисе Atlas Media API. Виджет автоматически получает название станции,
...
Без параметров собирается вариант `plainRelease` без встроенного API:

```sh
ANDROID_HOME=/path/to/android-sdk sh gradlew --offline clean check assembleRelease
```

Чтобы одновременно получить `plainRelease` и `bundledRelease`, передайте путь к подписанному APK
Atlas Media API:
...
Файлы получают суффиксы `-plain-release.apk` и `-bundled-release.apk`. Встроенный APK добавляется
только в bundled build output и не хранится в репозитории. Android всё равно потребует разрешить
установку из Atlas Media Widget и отдельно подтвердить установку API.

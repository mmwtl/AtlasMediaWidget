...
Все четыре клиентских пункта реализованы в версии `1.0.10 (11)`. Проверки firmware-specific частей
...
- высота нижней панели, размер transport-иконок и их разбежка от центра регулируются отдельно
...
`1.0.10[11]AtlasMediaWidget`. Без локального `secure.signing.gradle` release APK остаётся unsigned.

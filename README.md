...
`protocol v1`. В варианте `integrated` медиасервис является компонентом самого Widget; варианты
`plain` и `bundled` работают с автономным Atlas Media API (`com.mmwtl.atlasmediaapi`).
...
- настройка текста, отступов, прогресса и панели управления;
...
- список лайкнутых радиостанций с обложками, сеткой от 2×2 до 4×4 и прямым
  переключением внутри карточки;
- опциональная навигация кнопками назад/вперёд по всем сохранённым или только избранным
  станциям без поиска по эфиру;
- открытие активного медиаприложения или штатного экрана Radio, Bluetooth и USB;
- отображение только поверх HOME, точное положение относительно выбранного угла,
  перетаскивание карточки и отключаемые точки перемещения;
...
- вариант `integrated` либо автономная Atlas Media API (`com.mmwtl.atlasmediaapi`) с поддержкой
  Media Bridge `protocol v1`, `GET_RADIO_STATIONS` и `TUNE_RADIO`;
...
- доступ к уведомлениям для медиасервиса, если нужны данные Android-медиаплееров;
- для надёжного автозапуска — разрешение автозапуска и снятие ограничений фоновой работы прошивки ГУ.

## Установка и запуск

1. Установите `integrated` APK Atlas Media Widget. Для `plain` или `bundled` сначала установите и
   настройте автономную Atlas Media API.
2. Откройте Atlas Media Widget и в разделе «Системные разрешения» выдайте доступ поверх окон,
   к истории использования и контролю окон. В `integrated` там же выдаются доступ к уведомлениям
   медиаплееров и хранилищу для USB.
3. Отдельный ярлык «Atlas Media API» в `integrated` открывает диагностику и настройки медиасервиса.
4. Проверьте статус медиасервиса на экране настроек.
5. Задайте размер, угол привязки и отступы карточки.
6. Нажмите `Запустить`.
7. При необходимости включите автозапуск после загрузки ГУ.

Карточка появляется на домашнем экране. Для перемещения используйте точки `⋮` в правом верхнем
углу; после выбора позиции их можно скрыть в настройках. Нажатие на свободную область карточки
открывает текущий медиаисточник. Когда активно Радио, правая плашка «Избранное» открывает внутри
карточки список лайкнутых станций. Экраны выбора источника и станции автоматически закрываются
через 10 секунд. Режим прямого
переключения кнопками назад/вперёд включается отдельно в разделе «Содержимое и поведение».

## Каталог радио

Каталоги радиостанций (встроенный каталог Пензы и импорт пользовательских архивов) управляются
централизованно во встроенном или автономном сервисе Atlas Media API. Виджет получает название станции,
частоту и обложку через Media Bridge.
Сохранённые и лайкнутые станции также поступают из Atlas Media API; виджет не хранит отдельную
копию этих списков и не запускает поиск перед прямой настройкой.

Формат архива и архитектура каталога описаны в [документации каталога](docs/radio-catalog.md).
...
масштаб интерфейса, режим навигации по радиостанциям и выбранный список, размер сетки избранного,
видимость точек перемещения и автозапуск. Системные
разрешения и текущее состояние сервиса в резервную копию не входят.
...
Release APK создаются в `app/build/outputs/apk/<variant>/release/` с именами вида
`<versionName>[<versionCode>]AtlasMediaWidget-<variant>-release.apk`.

Без параметров собираются `plainRelease` с автономным API и `integratedRelease` со встроенным
медиасервисом:
...
Чтобы дополнительно получить `bundledRelease`, передайте путь к подписанному APK Atlas Media API:
...
Файлы получают суффиксы `-plain-release.apk`, `-integrated-release.apk` и
`-bundled-release.apk`. Вариант `integrated` содержит backend как приватный сервис в процессе
`:media` и не устанавливает второй пакет. Вложенный APK добавляется только в `bundled` и не
хранится в репозитории; Android отдельно попросит подтвердить его установку.
